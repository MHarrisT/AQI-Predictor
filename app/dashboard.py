...
st.markdown("<h1 class='title-text'>🌬️ Lahore AQI Predictor</h1>", unsafe_allow_html=True)
st.markdown("<p class='subtitle-text'>Real-time AI Air Quality forecasting powered by XGBoost & Hopsworks</p>", unsafe_allow_html=True)
...
def get_aqi_details(aqi):
    ...
    val = round(aqi)
    if val <= 1: return "#10B981", "Good"
    elif val == 2: return "#F59E0B", "Fair"
    elif val == 3: return "#F97316", "Moderate"
    elif val == 4: return "#EF4444", "Poor"
    else: return "#8B5CF6", "Very Poor"
...
with st.spinner("Fetching high-resolution AI predictions..."):
    try:
        ...
        if response.status_code == 200:
            ...
            st.markdown("### 📍 Current Forecast Details (Today)", unsafe_allow_html=True)
            ...
            with col1:
                ...
                fig_gauge = go.Figure(go.Indicator(
                    ...
                    number = {'font': {'size': 60, 'color': color}, 'valueformat': '.1f'},
                    gauge = {
                        ...
                        'borderwidth': 0,
                        'steps': [
                            {'range': [1, 1.5], 'color': "rgba(16, 185, 129, 0.15)"},
                            {'range': [1.5, 2.5], 'color': "rgba(245, 158, 11, 0.15)"},
                            {'range': [2.5, 3.5], 'color': "rgba(249, 115, 22, 0.15)"},
                            {'range': [3.5, 4.5], 'color': "rgba(239, 68, 68, 0.15)"},
                            {'range': [4.5, 5], 'color': "rgba(139, 92, 246, 0.15)"}
                        ]
                    }
                ))
                fig_gauge.update_layout(font={'family': "Outfit"}, height=350, margin=dict(t=50, b=0))
                st.plotly_chart(fig_gauge, use_container_width=True)
                
                st.markdown(f"<h3 style='text-align: center; color: {color}; margin-top: -30px;'>{status}</h3>", unsafe_allow_html=True)

            with col2:
                st.markdown("<br>", unsafe_allow_html=True)
                st.markdown("#### Weather & Pollutant Breakdown")
                st.markdown(f"<p style='color: var(--text-color); opacity: 0.8;'>Temperature: <b>{today_data['temp']}°C</b> &nbsp;|&nbsp; Humidity: <b>{today_data['humidity']}%</b></p>", unsafe_allow_html=True)
            ...
            # Chart
            st.markdown("<br><h3 style='font-weight: 600;'>📈 4-Day Forecast Trend</h3>", unsafe_allow_html=True)
            df = pd.DataFrame(forecast)
            
            fig = px.area(df, x="date", y="predicted_aqi", markers=True, 
                          labels={"date": "Forecast Date", "predicted_aqi": "Predicted AQI Score"})
            
            fig.update_traces(line_color="#06b6d4", fillcolor="rgba(6, 182, 212, 0.15)", 
                              line_width=4, marker=dict(size=12, color="#10b981", line=dict(width=2, color="white")))
        ...
        st.error("Backend API is not running. Please start the FastAPI server with `python -m uvicorn app.api:app --reload`.")
