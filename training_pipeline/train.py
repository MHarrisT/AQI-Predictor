...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # 3 = FATAL only (silences info, warnings, and errors)
...
def fetch_and_preprocess_data():
    ...
    return project, X_train_scaled, X_test_scaled, y_train, y_test, scaler
...
def train_and_evaluate(X_train, X_test, y_train, y_test):
    ...
    rf = RandomForestRegressor(n_estimators=100, max_depth=5, min_samples_split=10, min_samples_leaf=5, random_state=42, n_jobs=-1)
    ...
    # Add regularization parameters to prevent overfitting
    xgb = XGBRegressor(n_estimators=100, learning_rate=0.05, max_depth=4, subsample=0.8, colsample_bytree=0.8, reg_alpha=1.0, reg_lambda=1.0, random_state=42)
    ...
    rnn.fit(X_train_rnn, y_train, epochs=30, batch_size=32, validation_split=0.2, callbacks=[es], verbose=0)
...
def main():
    # 1. Fetch & Preprocess
    project, X_train, X_test, y_train, y_test, scaler = fetch_and_preprocess_data()
